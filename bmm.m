...
perplexity_K = zeros(length(K), 1); %stores perplexity for different K

for t = 1:length(K)

%alpha = 0.1;                 % parameter of the Dirichlet over mixture components
alpha = 10;    %original value of alpha
gamma = 0.1;                % parameter of the Dirichlet over words

% Initialization: assign each document a mixture component at random
sd = ceil(K(t)*rand(D,1));     % mixture component assignment of each document
swk = zeros(W,K(t));           % K multinomials over W unique words
sk_docs = zeros(K(t),1);            % number of documents assigned to each mixture component
...
iterations = 10;
perplexity_Gibbs = zeros(size(iterations,2),1);

for n = 1:length(iterations)

theta = zeros(K(t), size(iterations,2));
proportions = zeros(K(t), iterations(n));

for iter = 1:iterations(n)     % number of Gibbs sweeps
  for d = 1:D       % for each document iterate through all its words
    ...
    lb = zeros(1,K(t));    % log probability of doc d under mixture component k
    for k = 1:K(t)
  ...
  proportions(:,iter) = sk_docs/D;    %proportion of docs assigned to each component
...
I = 20;
for k=1:K(t), [i ii] = sort(-swk(:,k)); ZZ(k,:)=ii(1:I); end
for i=1:I, for k=1:K(t), fprintf('%-15s',V{ZZ(k,i)}); end; fprintf('\n'); end


perplexity_K(t,1) = perplexity;

perplexity_Gibbs(n,1) = perplexity;
...
figure(3)
plot(perplexity_Gibbs);


% POSSIBLE PLOTS FOR ANALYSIS
% 1. PERPLEXITY VS NUMBER OF TOPICS
% 2. ACCURACY VS PROPORTION OF DATA USED FOR TRAINING
% 3. PREDICTIVE PERPLEXITY VS NUMBER OF TOPICS

% 4. PERPLEXITY VS NUMBER OF GIBBS SWEEPS 

% 5. CHANGE THE VALUES OF ALPHA AND BETA AND OBSERVE RESULTS
